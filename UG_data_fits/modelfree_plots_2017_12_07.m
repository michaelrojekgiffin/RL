...
for oo = 1:size(ugm, 2)
    for tt = 1:size(ugm, 3)
        ugm(1, oo, tt) = mean(soc_imp(soc_imp(:, 5) == oo & soc_imp(:, 8) == tt, 1));          % social implicit
        ugm(2, oo, tt) = mean(nonsoc_imp(nonsoc_imp(:, 5) == oo & nonsoc_imp(:, 8) == tt, 1)); % non social implicit
...
%%
for ii = 1:3
    snugs(1, ii, :) = mean([ugm(1, ii, :), ugm(3, ii, :)]);
    snugs(2, ii, :) = mean([ugm(3, ii, :), ugm(3, ii, :)]);
end
...
for ii = 1:3
    ...
    plot(1:40, squeeze((snugs(1, ii, :))), 'markers',5, 'linewidth', 1)
end

nugs = mean([ugm(2, :, :), ugm(4, :, :)]);
