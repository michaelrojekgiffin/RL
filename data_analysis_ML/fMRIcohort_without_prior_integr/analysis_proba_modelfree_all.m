...
for k = 1:nsub;
    ...
    for kcond = 1:3
        ...
        for kSoc = 1:2
            ...
            [b(k,kcond,:,kSoc)] = glmfit(XX(:,1),XX(:,2)./100,'binomial');
            mod_mat(k,kcond,:,kSoc) = 100.*glmval(squeeze(b(k,kcond,:,kSoc)),XX(:,1),'logit');
        end
    end
...
for kSoc = 1:2
    ...
    for k = 1:3
        ...
        xmtp = squeeze(mean(mod_mat(:,k,:,kSoc),1));
        xstp =  squeeze(std(mod_mat(:,k,:,kSoc),0,1))./nsub;
        
        fill([1:21,fliplr(1:21)],[xmtp'+xstp',fliplr(xmtp'-xstp')],colmat(k,:),...
            'LineStyle','none')
        alpha(.5)
