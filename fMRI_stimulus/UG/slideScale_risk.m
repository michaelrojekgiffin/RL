function [position, RT, answer, offer_start_position] = slideScale_risk(numFrames, screenPointer, question, rect, endPoints, varargin)
...
if drawImage == 1
    rectImage  = [center(1) - imageSize(2)/1.3, rect(4)*(scalaPosition - 0.3) - imageSize(1), center(1) + imageSize(2)/1.3, rect(4)*(scalaPosition - 0.7)+imageSize(1)];
...
for tickem = 1:length(all_ticks)
    ...
    xticker = xticker + (length(scaleRange)/6);
end
...
%     end
    ...
    if keyCode(leftKey) == 1 && position > 1
        x = x - (length(scaleRange)/6);
        position = position - 1;
    elseif keyCode(rightKey) == 1 && position < 6
        x = x + (length(scaleRange)/6);
        position = position + 1;
    end
