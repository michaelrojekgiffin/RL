...
while samesame == 0
    ...
    for ii = 1:length(all_idx)
        ...
        for ss = 1:length(tmpdir)
            samecount = samecount + startsWith(tmpdir(ss).name, 'square');
            samecount = samecount + startsWith(tmpdir(ss).name, 'trapazoid');
            samecount = samecount + startsWith(tmpdir(ss).name, 'diamond');
        end
        if samecount > 1
            samesame = 0;
        end
        ...
        samecount = 0;
        for ss = 1:length(tmpdir)
            samecount = samecount + startsWith(tmpdir(ss).name, 'circle');
            samecount = samecount + startsWith(tmpdir(ss).name, 'crescent');
            samecount = samecount + startsWith(tmpdir(ss).name, 'clover');
        end
