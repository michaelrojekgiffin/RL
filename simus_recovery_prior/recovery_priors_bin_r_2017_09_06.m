...
tr_choice = repmat(nchoosek(0:10,2),10,1);
...
for k_sub   = 1:nsub
    ...
    ll_rep          = NaN(n_rep,1);
    for k_rep = 1:n_rep
        x0 = [-5*rand() 5*rand()  5*rand() .5+rand();];
        [parameters_rep(k_rep,1:4),ll_rep(k_rep,1)]=fmincon(@(x) estimate_priors_bin_r(x,tr_choice,sub_o(k_sub,:)),x0,[],[],[],[],lb,ub,[],options);
    end
    [~,pos]                  = min(ll_rep);
...
for k = 1:4
    ...
    [R(k),P(k)]= corr(PARAMS(:,k),parameters(:,k));
end
