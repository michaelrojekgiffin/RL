...
function [negLL, EV, PA, V] = ModelEstimation_2params_2017_09_11(params,priors, o,r,model, predprey)
% comp params
beta1   = params(1); % choice temperature
lr1     = params(2); % supraliminal learning rate

V0      = priors(1); % initial intercept
beta2   = priors(2); % slope of opponent's acceptance function
...
% V       = NaN(ntrial); % estimated intecept of decision function
EPc     = NaN(ntrial, 1); % estimated expected value of all offers
CPE     = NaN(ntrial, 1); % estimated expected value of all offers
...
for i = 1:ntrial
    ...
    if model==1
        ...
        CPE(i)      = r(i) - EPc(i);
        % DANGER - I changed the below from add to subtract
        % for some reason, if I increase the threshold the subject appears
        % to learn in predator, but if I decrease the threshold the subject
        % appears to learn in prey
        switch predprey
            case 'predator'
