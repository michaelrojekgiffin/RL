...
v0rnd      = -10 + 10*rand(nsims,1); %  proposer initial prior on threshold (to be fitted or estimated from no-feedback games)
B0rnd      = 10*rand(nsims,1);       %  proposer rating temperature
a0rnd      = 10*rand(nsims,1);       %  proposer learning rate

Brnd       = 5*rand(nsims,1);       %  proposer noise (proposer's estimate of slope of opponent, to be fitted or estimated from no-feedback games)
...
for k_sim = 1:nsims
    ...
    v0 = v0rnd(k_sim);  % proposer estimate of receiver threshold (intercept)
    B0 = B0rnd(k_sim);  % proposer temperature 
    a0 = a0rnd(k_sim);  % proposer learning rate
    B  = Brnd(k_sim);   % proposer estimate of receiver threshold (slope)
...
%     legend('V','O','D')
    ...
    for k_rep = 1:n_rep     
        x0 = lb + rand(1,4).*ddb;
        [parameters_rep(k_rep,1:4),ll_rep(k_rep,1)]=fmincon(@(x) ModelEstimation3_Full_2016_11(x,O,D,1,predprey),x0,[],[],[],[],lb,ub,[],options);
    end
...
[RR, PP] = corrcoef(a0rnd,parameters(:,3));
txt1 = sprintf('r = %f\n p = %f', RR(2), PP(2));
text(mean(a0rnd), mean(parameters(:,3)), txt1);
