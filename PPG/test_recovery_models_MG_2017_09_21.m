...
%------------------------------------------
cond2learn  = -[9.5];%-[12,9,6,3,0];                        % all the intercepts the player needs to learn
nc          = numel(cond2learn);
...
for predprey_count = 1:length(predprey_array)
    ...
    for model_iter = 1:length(nmodel_array)
        ...
        for k_sim = 1:n_sims
            ...
            % generate the data
            [O,D,PE,at, pd, R_o] = learning_models_timeseries_MG_2017_09_21([bX,lr1,lr2],[Ra;Rb],n_trial,a0,b0,nmodel, predprey);
