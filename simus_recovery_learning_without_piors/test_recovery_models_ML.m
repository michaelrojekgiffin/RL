...
Pb_rnd          = 1+3*rand(n_sims,1);       %  Proposer  estimated accpetance noise (to be fitted or estimated from no-feedback games)
Px_rnd          = 1+3*rand(n_sims,1);         %  Proposer  rating temperature
...
parameters  = NaN(n_sims,5);        parametersLPP  = NaN(n_sims,5);
...
for k_sim = 1:n_sims
    ...
    ub = [5 0 5 3 1];         UB = [Inf Inf Inf 3 1];
    ddb = ub - lb;
...
PP = [Px_rnd,Pa_rnd,Pb_rnd,Plr1_rnd,Plr2_rnd];           %  Proposer  learning rate
legB = {'rating temperature','Accept Thresh','Accept Slope','learning rate 1','learning rate 2'};
